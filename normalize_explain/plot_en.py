...
for file_name_1, file_name_2, frequency_width, filename in plot_file_path:
    ...
    plt.figure(figsize=(12, 8))  # サイズを大きめに設定
    plt.plot(plot_data_1, lw=2, color='blue', label='Device 1 (Signal)', linestyle='-')  # 実線
    plt.plot(plot_data_2, lw=2, color='orange', label='Device 2 (Signal)', linestyle='--')  # 点線

    # グリッドを追加
    plt.grid(visible=True, linestyle='--', linewidth=0.5, alpha=0.7)

    # タイトルの追加
    # plt.title(f"Signal Strength Over Time ({frequency_width} MHz)\nCorrelation: {corr}, DTW: {dtw}",
    #           fontsize=24, fontweight='bold')

    # テキスト情報を図内に追加
    plt.text(x=len(data1_1) * 0.05, y=max(plt.ylim()) * 0.95,
             s=f"Correlation Coefficient: {corr}",
             fontsize=20, bbox=dict(facecolor='white', edgecolor='gray', boxstyle='round,pad=0.5'))

    # x軸とy軸ラベル
    plt.xlabel("Time (seconds)", fontsize=24, fontweight='bold')
    plt.ylabel("Normalized Signal Strength", fontsize=24, fontweight='bold')

    # x軸の目盛りをカスタマイズ
    x_ticks = np.linspace(0, len(data1_1), 6)
    x_labels = [f"{int(i)}" for i in np.linspace(0, 30, 6)]  # 秒単位のラベル
    ...
    # 軸の範囲を動的に設定
    plt.xlim([0, len(data1_1)])
    plt.ylim([min(min(plot_data_1), min(plot_data_2)) * 1.1, max(max(plot_data_1), max(plot_data_2)) * 1.1])

    # 凡例を追加
    plt.legend(fontsize=24, loc='upper right', frameon=True, shadow=True)

    # プロット作成後、かつ保存前に現在のAxesを取得
    ax = plt.gca()
    # x軸とy軸の目盛りラベルのパディングを設定（数値はお好みで調整してください）
    ...
    plt.savefig(f'./fig/en/{filename}_en.pdf', format='pdf')
