...
def create_graph(df, filename_suffix=''):
    ...
    for i, (w, p) in enumerate(zip(witness_times, proof_times)):
        ...
        # 合計を上に表示
        ax.text(i, total + 0.15, f'{total:.3f}秒', ha='center', va='bottom', fontsize=20, fontweight='bold')
    ...
    # タイトル
    ax.set_title('snarkjs', fontsize=28, fontweight='bold')
...
def create_proof_only_graph(df, filename_suffix=''):
    ...
    for i, p in enumerate(proof_times):
        ax.text(i, p + 0.05, f'{p:.3f}秒', ha='center', va='bottom', fontsize=20, fontweight='bold')
    ...
    # タイトル
    ax.set_title('snarkjs (Proof生成のみ)', fontsize=28, fontweight='bold')
...
def create_mac_comparison_graph(df):
    ...
    for i, (w, p) in enumerate(zip(witness_times, proof_times)):
        ...
        if p > 0.2:
            ax.text(i, w + p/2, f'{p:.3f}', ha='center', va='center', fontsize=18, fontweight='bold', color='white')
        # 合計を上に表示
        ax.text(i, total + 0.1, f'{total:.3f}秒', ha='center', va='bottom', fontsize=20, fontweight='bold')
    ...
    # タイトル
    ax.set_title('snarkjs (Mac vs Mac Docker)', fontsize=28, fontweight='bold')
